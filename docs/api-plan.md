...
`POST /api/puzzle/past` returns a signed, untracked puzzle token for a previous official daily puzzle. Dates must be earlier than today and no earlier than `2021-06-19`.
...
`POST /api/friends-family/login` checks the access code plus first name and last initial, upserts the matching family user, makes the new session active, and returns a signed identity token. The token contains identity metadata but no access code.
...
`POST /api/friends-family/stats` returns full-page stats dashboard data: family overview, per-user summaries, starter-word counts, daily history, timeline data, leaderboard data, and solve analysis. If the requesting user has not solved the current daily puzzle, other players' current-day answer, guesses, board, and analysis are replaced with locked placeholders.
