...
`POST /api/puzzle/past` returns a signed, untracked Wordle puzzle token for a previous official daily Wordle. Submitted dates before `2021-06-19` are clamped to the first playable puzzle, and submitted dates for today or later are clamped to yesterday.
...
`POST /api/friends-family/today-status` returns whether the signed-in family user has already completed the requested daily Wordle, plus their stored result when complete or their active in-progress attempt when not complete.

`POST /api/friends-family/stats` returns full-page stats dashboard data: family overview, per-user summaries, starter-word counts, capped daily history, capped timeline data, leaderboard data, and derived solve analysis. If the requesting user has not solved the current daily Wordle, other players' current-day answer, guesses, board, and analysis are replaced with locked placeholders.
...
Random and past-date Wordle results return session-only solve analysis but are not inserted into stats tables.
