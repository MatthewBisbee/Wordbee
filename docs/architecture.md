...
Daily Wordle answer flow:

- `GET /api/today` resolves the current puzzle date in the configured puzzle timezone, defaulting to `America/Chicago` so the active daily flips at Central midnight even if the next official answer is already fetchable.
...
- Direct daily-answer dates before `2021-06-19` are rejected because that is the first official playable date; past-date Wordle play clamps submitted out-of-range dates to `2021-06-19` through yesterday.
